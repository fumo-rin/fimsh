...
namespace Pinwheel.Poseidon
{
    ...
    public class PWaterInspector : Editor
    {
            ...
            public static GUIStyle BoxStyle
            {
                get
                {
                    if (boxStyle==null)
                    {
                        ...
                        boxStyle.margin = new RectOffset(0, 0, 20, 20);
                        boxStyle.padding = new RectOffset(8, 8, 8, 12);
                    }
                    return boxStyle;
                }
            }
        ...
        private void DrawPoseidon2Notice()
        {
            Rect r = EditorGUILayout.BeginVertical(P2GUI.BoxStyle);
            PEditorCommon.DrawOutlineBox(r, Color.white);
            EditorGUILayout.LabelField("<b>Poseidon 2</b> is available with reworked structure and workflow, upgrade for free.", PEditorCommon.WordWrapLeftLabel);
            if (EditorGUILayout.LinkButton("Learn more →"))
            {
                NetUtils.TrackClick("learn_more_poseidon2", UILocation.Inspector);
                Application.OpenURL("https://docs.pinwheelstud.io/poseidon/2/docs/upgrade-guide.html");
            }
